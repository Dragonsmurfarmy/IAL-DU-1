...
void Queue_Init( Queue *queue ) {
	/*	if queue param is NULL, error occures
		else initializes values of all array nodes to "*" and indexes to 0
	*/
	if(queue == NULL){
		Queue_Error(QERR_INIT);
	}
...
int nextIndex( int index ) {
	//returns index of next array slot
...
int Queue_IsEmpty( const Queue *queue ) {
	//returns non-zero value for empty queue, 0 value if it is not empty
...
int Queue_IsFull( const Queue *queue ) {
	//returns non zero value for full queue, 0 value if it is not full
...
void Queue_Front( const Queue *queue, char *dataPtr ) {
	//if queue is not empty, returns value of first index in dataPtr variable, otherwise error occures
...
void Queue_Remove( Queue *queue ) {
	//if queue is not empty, removes value of 1st index, if it is, error occures
	if(Queue_IsEmpty(queue)){
...
void Queue_Dequeue( Queue *queue, char *dataPtr ) {
	//if queue is not empty,stores value of 1st index in dataPtr variable, then
	//removes value of 1st index, if queue is empty, error occures
	if(Queue_IsEmpty(queue)){
...
void Queue_Enqueue( Queue *queue, char data ){
	//if queue is not empty,stores value of data variable in free index, then
	//moves value of freeindex to the following index, if queue is empty, error occures
